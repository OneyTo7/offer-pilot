...
public class ReportServiceImpl implements ReportService {
    ...
    private void generateStubReport(Report report) {
        ...
        report.setTechStackAnalysis(JSONUtil.toJsonStr(
            ReportContent.TechStackAnalysis.builder().matched("Java, Spring Boot, MySQL, MyBatis-Plus, Redis")
                .missing("Docker, Kubernetes, 微服务架构, 消息队列").recommendation("建议补充容器化和微服务相关经验")
                .build()));
        report.setHighlights(JSONUtil.toJsonStr(
            List.of("5年Java开发经验，主导过多个大型项目", "熟悉Spring Boot生态，有微服务架构设计经验",
                "具备良好的系统设计能力和团队协作能力")));
        report.setWeaknesses(JSONUtil.toJsonStr(
            List.of("缺乏云原生技术栈（Docker/K8s）的实战经验", "对分布式事务和消息队列的理解需要加强")));
